...
namespace Codefarts.GridMapping
{
    ...
    public static class GridMapExtensionMethods
    {
        ...
        {
            var locked = map.LayerLocked;
            var names = map.LayerNames;
            var visibility = map.LayerVisibility;
            Array.Resize(ref locked, locked.Length + 1);
            Array.Resize(ref names, names.Length + 1);
            Array.Resize(ref visibility, visibility.Length + 1);
            visibility[map.LayerVisibility.Length - 1] = true;
            names[map.LayerNames.Length - 1] = string.Empty;
            map.LayerLocked = locked;
            map.LayerNames = names;
            map.LayerVisibility = visibility;
        }
        ...
        public static void SetLayerCount(this GridMap map, int count)
        {
            ...
            while (map.LayerLocked.Length != count)
            {
                var locked = map.LayerLocked;
                var names = map.LayerNames;
                var visibility = map.LayerVisibility;
                ...
                Array.Resize(ref locked, Math.Max(1, count));
                Array.Resize(ref names, Math.Max(1, count));
                Array.Resize(ref visibility, Math.Max(1, count));
        ...
        {
            var names = new string[map.LayerNames.Length];
            map.LayerNames.CopyTo(names, 0);
        ...
        {
            var newIndex = index + count;
            newIndex = newIndex > map.LayerNames.Length - 1 ? map.LayerNames.Length - 1 : newIndex;
            if (newIndex == index)
            {
                return;
            }
        ...
        public static void SetLayerVisibility(this GridMap map, int index, bool visible, Action<GameObject[]> callback)
        {
            ...
            map.LayerVisibility[index] = visible;
        }
        ...
        public static void DeleteLayer(this GridMap map, int index)
        {
            ...
            if (map.LayerNames.Length > 1 && index > 0)
            {
                Array.Copy(map.LayerNames, index, map.LayerNames, index - 1, map.LayerNames.Length - index);
                Array.Copy(map.LayerVisibility, index, map.LayerVisibility, index - 1, map.LayerVisibility.Length - index);
                Array.Copy(map.LayerLocked, index, map.LayerLocked, index - 1, map.LayerLocked.Length - index);
            }

            SetLayerCount(map, map.LayerNames.Length - 1);
        }
        ...
        public static void SwapLayers(this GridMap map, int index, int newIndex, Action<SwapModel> callback, Action<SwapModel> completedCallback)
        {
            ...
            var nameValue = map.LayerNames[index];
            map.LayerNames[index] = map.LayerNames[newIndex];
            map.LayerNames[newIndex] = nameValue;

            var boolValue = map.LayerVisibility[index];
            map.LayerVisibility[index] = map.LayerVisibility[newIndex];
            map.LayerVisibility[newIndex] = boolValue;

            boolValue = map.LayerLocked[index];
            map.LayerLocked[index] = map.LayerLocked[newIndex];
            map.LayerLocked[newIndex] = boolValue;
            ...
            bool autoCenterPrefab, bool autoScalePrefab, Func<int, int, int, GameObject, bool> callback)
        {
            ...
            var getClone = new Func<bool, GameObject>(first =>
                {
                    GameObject value;
                    if (first)
                    {
            ...
            for (var rowIndex = 0; rowIndex < height; rowIndex++)
            {
                ...
                // left side
                var clonedObject = getClone(firstRun);
                firstRun = false;
                ...
                {
                    // right side
                    clonedObject = getClone(firstRun);
