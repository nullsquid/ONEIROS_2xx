...
namespace Codefarts.GridMapping
{
    ...
    public class PerformanceTesting<T>
    {
        ...
        /// </summary>
        /// <remarks>
        /// If you had two performance counter instances PerformanceTesting{int}.Instance as well as PerformanceCounter{string}.Instance you can reset all
        /// valuse for both types by calling
        /// <code>
        ///  PerformanceTesting{int}.ResetAll(new[] { typeof(int), typeof(string) });
        /// </code>
        /// </remarks>
        /// <param name="types">One or more types that the performance counter uses as it's key.</param>
        public static void ResetAll(Type[] types)
        {
            ResetAll(types, false);
        }
        ...
        public static void ResetAll(Type[] types, bool resetCounts)
        {
            foreach (var t in types)
            {
                DoResetAll(t, resetCounts);
            }
        }
        ...
        /// </summary>
        /// <typeparam name="TX">The type that the performance counter uses as it's key.</typeparam>
        /// <param name="type">The type that the performance counter uses as it's key.</param>
        /// <param name="resetCounts">If true will set each timer start count to 0.</param>
        internal static void DoResetAll<TX>(TX type, bool resetCounts)
        {
            PerformanceTesting<TX>.Instance.ResetAll(resetCounts);
        }
        ...
        public Func<string, T> ConvertFromStringCallback { get; set; }
        public Func<T, string> ConvertToStringCallback { get; set; }
