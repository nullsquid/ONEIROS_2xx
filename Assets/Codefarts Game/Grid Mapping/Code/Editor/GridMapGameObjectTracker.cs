...
namespace Codefarts.GridMapping
{
    ...
    /// <summary>
    /// Provides a game object tracking for a grid map.
    /// </summary>
    public class GridMapGameObjectTracker
    {
        ...
        /// <summary>
        /// Flag used to determine if a scan is currently working.
        /// </summary>
        private bool processing;

        /// <summary>
        /// Flag used to inform the scan that it should stop.
        /// </summary>
        private bool cancel;

        /// <summary>
        /// Holds the scan progress.
        /// </summary>
        private float progress;


        /// <summary>
        /// Initializes a new instance of the <see cref="GridMapGameObjectTracker"/> class.
        /// </summary>
        /// <param name="map">The map that will be scaned and tracked.</param>
        /// <param name="scan">If set to <c>true</c> will perform a scan.</param>
        /// <param name="background">If set to <c>true</c> will scan in the background.</param>
        ...
        public void Scan(bool background)
        {
            ...
            if (background)
            {
                // this.map.StartCoroutine(this.DoScan(0));
        ...
        private void DoScan(int sleep)
        {
            ...
            this.objects = new Array3D<GameObject>(this.map.Columns, this.map.Rows, Math.Max(this.map.LayerNames.Length, 1));
