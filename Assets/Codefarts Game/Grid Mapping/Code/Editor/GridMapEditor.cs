...
namespace Codefarts.GridMapping.Editor
{
    ...
    public partial class GridMapEditor : Editor
    {
        ...
        /// <summary>
        /// If true the prefab types will be visible.
        /// </summary>
        private bool showPrefabTypes = true;

        /// <summary>
        /// Determines weather or not the rotation controls are visible.
        /// </summary>
        private bool showRotationControls = true;

        /// <summary>
        /// Determine wheather or not the recent material list will be visible.
        /// </summary>
        private bool showRecentMaterialList = true;

        /// <summary>
        /// Determines wheather or not the recent prefab list will be visible.
        /// </summary>
        private bool showRecentPrefabList=true;

        /// <summary>
        /// Determines weather or not hte rotation controls are visible.
        /// </summary>
        public bool ShowRotationControls
        {
            get
            {
                return this.showRotationControls;
            }

            set
            {
                this.showRotationControls = value;
            }
        }
        ...
        /// Determines wheather or not the recent prefab list will be visible.
        /// </summary>
        public bool ShowRecentPrefabList
        {
            get
            {
                return this.showRecentPrefabList;
            }

            set
            {
                this.showRecentPrefabList = value;
            }
        }
        ...
        /// Determine wheather or not the recent material list will be visible.
        /// </summary>
        public bool ShowRecentMaterialList
        {
            get
            {
                return this.showRecentMaterialList;
            }

            set
            {
                this.showRecentMaterialList = value;
            }
        }
        ...
        /// If true the prefab types will be visible.
        /// </summary>
        public bool ShowPrefabTypes
        {
            get
            {
                return this.showPrefabTypes;
            }

            set
            {
                this.showPrefabTypes = value;
            }
        }
        ...
        public int ActiveLayer
        {
            ...
            set
            {
                ...
                value = value > map.LayerNames.Length - 1 ? map.LayerNames.Length - 1 : value;
        ...
        public string ActiveLayerName
        {
            get
            {
                ...
                return map.LayerNames[this.activeLayer];
            }
            ...
            set
            {
                ...
                map.LayerNames[this.activeLayer] = value;
            }
        }
        ...
        public void OnDisable()
        {
            ...
            var contentForXAxis = this.rotationContentForXAxis;
            if (contentForXAxis != null)
            {
                foreach (var content in contentForXAxis)
                {
                    try
                    {
            ...
            var contentForYAxis = this.rotationContentForYAxis;
            if (contentForYAxis != null)
            {
                foreach (var content in contentForYAxis)
                {
                    try
                    {
                        Resources.UnloadAsset(content.image);
            ...
            var contentForZAxis = this.rotationContentForZAxis;
            if (contentForZAxis != null)
            {
                foreach (var content in contentForZAxis)
                {
                    try
                    {
                        Resources.UnloadAsset(content.image);
        ...
        private void DrawGizmos()
        {
            ...
            Handles.DrawLine(
                ...
                new Vector3(this.highlightPosition.X, (map.LayerNames.Length * map.Depth) - (map.Depth / 2f), this.highlightPosition.Z));
        ...
        private void DrawLayerList(bool scrollable)
        {
            ...
            if (scrollable && map.LayerNames.Length > 3)
            {
                var height = Math.Min((map.LayerNames.Length * layerIconSize) + 2, listHeight);
            ...
            switch (layerStyle)
            {
                ...
                case 1:
                    ...
                    GUILayout.BeginVertical();
                    for (var i = 0; i < map.LayerNames.Length; i++)
                    {
                        var layerLocked = map.LayerLocked[i];
                        var layerName = map.LayerNames[i];
                        var layerVisible = map.LayerVisibility[i];
                        GUILayout.BeginHorizontal(GUILayout.ExpandHeight(false));
                        GUILayout.Space(4);

                        if (showLocks)
                        {
                            var content = this.layerStyles[layerLocked ? 2 : 3];
                            GUI.SetNextControlName(string.Format("chkLayerLock{0}", i));
                            layerLocked = GUILayout.Toggle(layerLocked, string.Empty, content, GUILayout.Width(layerIconSize), GUILayout.MaxWidth(layerIconSize), GUILayout.Height(layerIconSize), GUILayout.MaxHeight(layerIconSize));
                            map.LayerLocked[i] = layerLocked;
                        }

                        if (showVisibility)
                        {
                            var content = this.layerStyles[layerVisible ? 1 : 0];
                            GUI.SetNextControlName(string.Format("icoLayerVisible{0}", i));
                            var visible = GUILayout.Toggle(layerVisible, string.Empty, content, GUILayout.Width(layerIconSize), GUILayout.MaxWidth(layerIconSize), GUILayout.Height(layerIconSize), GUILayout.MaxHeight(layerIconSize));
                            if (visible != layerVisible)
                            {
                                //Undo.RegisterSceneUndo(local.Get("LayerVisibility"));
                                Undo.RecordObject(map, local.Get("LayerVisibility"));
                                map.LayerVisibility[i] = visible;
                        ...
                        if (this.activeLayer == i)
                        {
                            ...
                            layerName = EditorGUILayout.TextField(layerName, GUILayout.Height(layerIconSize), GUILayout.MaxHeight(layerIconSize));
                            map.LayerNames[i] = layerName;
                        }
                        else
                        {
                            GUI.SetNextControlName(string.Format("btnLayerName{0}", i));
                            if (GUILayout.Button(layerName, GUILayout.Height(layerIconSize), GUILayout.MaxHeight(layerIconSize)))
                            {
                                this.ActiveLayer = i;
                        ...
                        if (showDelete && !layerLocked && map.LayerNames.Length > 1 && GUILayout.Button(this.buttonContent[2], GUI.skin.label, GUILayout.Width(layerIconSize), GUILayout.Height(layerIconSize), GUILayout.MaxHeight(layerIconSize)))
                        {
                            ...
                            if (!promptToDeleteLayer || (promptToDeleteLayer && EditorUtility.DisplayDialog(
                                ...
                                string.Format(local.Get("ConfirmDeleteLayer"), layerName),
                                local.Get("Accept"),
                                local.Get("Cancel"))))
                            {
        ...
        private void DrawMapLayerControls()
        {
            ...
            var layerCount = map.LayerNames.Length;
            switch (layerCountStyle)
            {
                case 0:
                    ...
                    {
                        EditorGUILayout.LabelField(local.Get("LayerCount"), map.LayerNames.Length.ToString());
                    }
                    else
                    {
                        layerCount = EditorGUILayout.IntField(local.Get("LayerCount"), map.LayerNames.Length);
                    }

                    layerCount = layerCount < 1 ? 1 : layerCount;
                    if (layerCount != map.LayerNames.Length)
                    {
                ...
                case 1:
                    ...
                    if (layerCountFieldDisabled)
                    {
                        GUILayout.Label(map.LayerNames.Length.ToString());
                    }
                    else
                    {
                        layerCount = EditorGUILayout.IntField(map.LayerNames.Length);
                    }
            ...
            this.activeLayer = this.activeLayer > map.LayerNames.Length - 1 ? map.LayerNames.Length - 1 : this.activeLayer;
        ...
        {
            if (!this.showRecentMaterialList)
            {
                return;
            }
            ...
            if (value != gridMappingService.CurrentMaterial)
            {
                ...
                if (value != null)
                {
                    ...
                    settings.SetValue(GlobalConstants.CurrentMaterialKey, AssetDatabase.AssetPathToGUID(Helpers.GetSourcePrefab(value)));
                }
                else
                {
        ...
        private void DrawRecentPrefabList()
        {
            ...
            if (value != gridMappingService.CurrentPrefab)
            {
                ...
                if (value != null)
                {
                    ...
                    // store current selection in settings
                    settings.SetValue(GlobalConstants.CurrentPrefabKey, AssetDatabase.AssetPathToGUID(Helpers.GetSourcePrefab(value)));
                }
                else
                {
